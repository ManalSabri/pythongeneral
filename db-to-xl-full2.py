...
def read_queries_from_file(filename):
    with open(filename, 'r') as file:
        content = file.read().strip()  # Remove any trailing whitespace or newlines
        sections = content.split(">>>>")[1:]  # Split by ">>>>" and ignore the first empty section
        queries = []
        for section in sections:
            split_section = section.split('\n', 1)
            if len(split_section) == 2:  # Ensure there's both a title and a query
                title, query = split_section
                queries.append((title.strip(), query.strip()))
    return queries
